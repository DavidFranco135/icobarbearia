...
const Partners: React.FC = () => {
  ...
  const inp = `w-full border p-4 rounded-xl outline-none font-bold text-xs transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-300 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]'}`;
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 h-full overflow-auto scrollbar-hide">
      ...
      {activeTab === 'parceiros' && (
        <>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              { label: 'Total Parceiros', value: stats.total, color: '#0D9488' },
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {enriched.map(p => (
              <div key={p.id} className={`rounded-[2rem] p-6 border hover:border-[#0D9488]/40 transition-all group ${themeCard}`}>
                {/* Imagem / Logo */}
                {(p.image || p.logo) && (
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    ...
                    {p.category && (
                      <span className="text-[8px] font-black uppercase tracking-widest text-[#0D9488] bg-[#0D9488]/10 px-2 py-0.5 rounded-full">
                        {p.category}
                      </span>
                    )}
                    {p.description && (
                ...
                <div className="grid grid-cols-3 gap-2 mb-4">
                  <div className={`p-3 rounded-xl text-center ${theme === 'light' ? 'bg-zinc-50 border border-zinc-200' : 'bg-white/5 border border-white/5'}`}>
                    ...
                    <p className="text-sm font-black text-[#0D9488]">{p.discount}%</p>
                  </div>
                  <div className={`p-3 rounded-xl text-center ${theme === 'light' ? 'bg-zinc-50 border border-zinc-200' : 'bg-white/5 border border-white/5'}`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="p-3 bg-[#0D9488] text-black rounded-xl hover:scale-105 transition-all"
                  >
                    <QrCode size={16} />
                  </button>
                </div>
                ...
                {p.monthlyFee && p.monthlyFee > 0 ? (
                  <p className="text-[9px] text-zinc-500 font-black mt-3 text-right">
                    Mensalidade: <span className="text-[#0D9488]">R$ {p.monthlyFee.toFixed(2)}</span>
                  </p>
                ) : null}
              </div>
            ))}
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className={`rounded-[2rem] p-6 border ${themeCard}`}>
              <DollarSign size={20} className="text-[#0D9488] mb-3"/>
              <p className="text-[9px] font-black uppercase tracking-widest text-zinc-500">Total Mensal</p>
              <p className="text-2xl font-black text-[#0D9488] font-display italic">R$ {totalMensalidades.toFixed(2)}</p>
            </div>
            <div className={`rounded-[2rem] p-6 border ${themeCard}`}>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-center gap-4">
                        {p.logo ? (
                          ...
                          <div className="w-12 h-12 rounded-2xl bg-[#0D9488]/10 flex items-center justify-center text-[#0D9488] font-black text-lg">
                      ...
                      <div className="text-right shrink-0">
                        <p className="text-xl font-black text-[#0D9488]">R$ {(p.monthlyFee || 0).toFixed(2)}</p>
                      ...
                      <div className="mt-4 pt-4 border-t border-red-500/10">
                        {cobrancaPayingId === p.id ? (
                            ...
                            <div className="grid grid-cols-4 gap-2">
                              {(['PIX','DINHEIRO','DEBITO','CREDITO']).map(m => (
                                <button key={m} onClick={() => setCobrancaPayMethod(m)}
                                  className={`py-2 rounded-xl text-[8px] font-black uppercase border-2 transition-all ${cobrancaPayMethod === m ? 'border-[#0D9488] bg-[#0D9488]/20 text-[#0D9488]' : theme === 'light' ? 'border-zinc-200 bg-zinc-50 text-zinc-400' : 'border-white/10 bg-white/5 text-zinc-500'}`}>
                          ...
                          <button onClick={() => { setCobrancaPayingId(p.id); setCobrancaPayMethod('PIX'); }}
                            className="w-full py-3 rounded-xl font-black text-[9px] uppercase bg-[#0D9488] text-black hover:bg-[#E5A86A] transition-all">
      ...
      {activeTab === 'beneficios' && (
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: 'Total Gerados', value: benefitStats.total, icon: Gift, color: '#0D9488' },
          ...
          <div className={`rounded-[2rem] p-8 border ${themeCard}`}>
            <h3 className={`text-lg font-black font-display italic mb-6 flex items-center gap-3 ${txt}`}>
              <BarChart3 size={20} className="text-[#0D9488]" /> Benefícios Usados por Parceiro
            </h3>
                  ...
                  return (
                    <div key={key}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs font-black text-[#0D9488]">{data.count} usos</span>
                      </div>
                  ...
                  {(clientBenefits || [])
                    ...
                    .map((b: ClientBenefit) => (
                      <tr key={b.id} className={`transition-colors ${theme === 'light' ? 'hover:bg-zinc-50' : 'hover:bg-white/[0.02]'}`}>
                        ...
                        <td className="px-6 py-4">
                          <p className="text-xs font-bold text-[#0D9488]">{b.partnerName || '—'}</p>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-[8px] font-black uppercase border ${
                            ...
                            b.status === 'DISPONIVEL' ? 'text-teal-400 bg-blue-400/10 border-blue-400/20' :
      ...
      {showQrModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/95 backdrop-blur-xl animate-in zoom-in-95">
          <div className={`w-full max-w-sm rounded-[3rem] p-10 space-y-8 border text-center shadow-2xl ${theme === 'light' ? 'bg-white border-zinc-200' : 'cartao-vidro border-[#0D9488]/20'}`}>
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-[#0D9488] mb-2">QR Code do Parceiro</p>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/95 backdrop-blur-xl animate-in zoom-in-95">
          <div className={`w-full max-w-lg rounded-[3rem] p-10 space-y-6 border shadow-2xl overflow-y-auto max-h-[90vh] scrollbar-hide ${theme === 'light' ? 'bg-white border-zinc-200' : 'cartao-vidro border-[#0D9488]/20'}`}>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  className={`w-full border p-4 rounded-xl outline-none font-bold text-xs transition-all appearance-none cursor-pointer ${theme === 'light' ? 'bg-zinc-50 border-zinc-300 text-zinc-900 focus:border-[#0D9488]' : 'bg-zinc-900 border-white/10 text-white focus:border-[#0D9488]'}`}
