    ...
    <div className="space-y-8 animate-in fade-in duration-500 h-full overflow-auto pb-20 scrollbar-hide">
      ...
      {(!products || products.length === 0) ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
          {products.map((p: any) => (
            <div key={p.id} className={`${cardClass} rounded-[2.5rem] overflow-hidden group relative transition-all hover:border-[#0D9488]/30`}>
              {/* Badge ativo/inativo */}
              ...
              <div className="p-6 space-y-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <p className="text-lg font-black text-[#0D9488] whitespace-nowrap">R$ {Number(p.price).toFixed(2)}</p>
                </div>
                {p.description && (
      ...
      {showModal && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl animate-in zoom-in-95">
          <div className={`w-full max-w-lg rounded-[3rem] shadow-2xl flex flex-col max-h-[92vh] ${isDark ? 'cartao-vidro border-[#0D9488]/10' : 'bg-white border border-zinc-200'}`}>
            ...
            <div className="flex-1 overflow-y-auto px-8 pb-4 space-y-5 scrollbar-hide">
              ...
              <div className="space-y-2">
                ...
                <div
                  className={`w-full aspect-video rounded-2xl overflow-hidden border-2 border-dashed flex items-center justify-center cursor-pointer relative group transition-all ${form.image ? 'border-transparent' : isDark ? 'border-white/10 hover:border-[#0D9488]/40' : 'border-zinc-300 hover:border-[#0D9488]'} bg-black`}
