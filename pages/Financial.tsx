    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-20 no-print h-full overflow-auto scrollbar-hide">
      ...
      <div className={`${cardClass} rounded-[2rem] p-6`}>
        <div className="flex items-center gap-3 mb-4">
          <Filter size={14} className="text-[#0D9488]" />
          ...
          {PERIODS.map(p => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${
                filterPeriod === p.key
                  ? 'bg-[#0D9488] text-black border-transparent'
                  : isDark ? 'bg-white/5 text-zinc-500 border-white/5 hover:border-[#0D9488]/30' : 'bg-zinc-100 text-zinc-500 border-zinc-200 hover:border-[#0D9488]/30'
              }`}
            >
              {p.label}
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div
          ...
          className={`${cardClass} rounded-[2rem] p-8 border-[#0D9488]/20 cursor-pointer hover:border-[#0D9488]/40 transition-all`}
        >
          <div className="w-12 h-12 bg-[#0D9488]/10 text-[#0D9488] rounded-2xl flex items-center justify-center mb-6"><TrendingUp size={24} /></div>
          <h3 className={`text-[10px] uppercase font-black tracking-widest ${isDark ? 'text-zinc-500' : 'text-zinc-500'}`}>Lucro do Período</h3>
          <p className={`text-3xl font-black mt-2 font-display italic ${metrics.lucro >= 0 ? 'text-[#0D9488]' : 'text-red-500'}`}>
        ...
        <div className={`${cardClass} rounded-[2rem] p-8 border-blue-500/20`}>
          <div className="w-12 h-12 bg-blue-500/10 text-teal-400 rounded-2xl flex items-center justify-center mb-6"><DollarSign size={24} /></div>
          <h3 className={`text-[10px] uppercase font-black tracking-widest ${isDark ? 'text-zinc-500' : 'text-zinc-500'}`}>Saldo Atual (Global)</h3>
          <p className={`text-3xl font-black mt-2 font-display italic ${metrics.saldoGlobal >= 0 ? 'text-teal-400' : 'text-red-500'}`}>
      ...
      <div className={`${cardClass} rounded-[2rem] p-8`}>
        <div className="flex items-center gap-3 mb-8">
          <UserCheck className="text-[#0D9488]" />
          ...
          {barberStats.map(stat => (
            <div key={stat.id} className={`p-6 rounded-2xl border ${isDark ? 'bg-white/5 border-white/10' : 'bg-zinc-50 border-zinc-200'}`}>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-[10px] uppercase font-black text-[#0D9488]">
                  <span>Comissão:</span>
          ...
          {barberStats.map((stat, idx) => (
            <div key={stat.id} className={`flex items-center gap-5 p-4 rounded-2xl border ${isDark ? 'bg-white/5 border-white/5' : 'bg-zinc-50 border-zinc-200'}`}>
              ...
              <div className="text-right flex-shrink-0">
                ...
                <p className="text-[8px] font-black text-[#0D9488] uppercase mt-0.5">R$ {stat.commission.toFixed(2)} comissão</p>
              </div>
            </div>
          ))}
          {barberStats.length === 0 && (
      ...
      {showAddModal && (
        <div className={`fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-xl animate-in zoom-in-95 ${isDark ? 'bg-black/95' : 'bg-black/70'}`}>
          <div className={`w-full max-w-md rounded-[2.5rem] shadow-2xl flex flex-col max-h-[90vh] ${isDark ? 'cartao-vidro border-[#0D9488]/20' : 'bg-white border border-zinc-200'}`}>
      ...
      {selectedEntry && (() => {
        ...
        return (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/80 backdrop-blur-xl animate-in zoom-in-95" onClick={e2 => { if (e2.target === e2.currentTarget) setSelectedEntry(null); }}>
            <div className={`w-full max-w-md rounded-[2.5rem] shadow-2xl border flex flex-col max-h-[90vh] ${isDark ? 'cartao-vidro border-[#0D9488]/20' : 'bg-white border-zinc-200'}`}>
              {/* Header */}
              <div className="p-8 pb-4 flex items-start justify-between shrink-0">
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-[#0D9488] mb-1">
              ...
              <div className="overflow-y-auto flex-1 px-8 pb-4 space-y-3">
                ...
                {linkedApp && (
                  <>
                    ...
                    <div className={`p-4 rounded-2xl space-y-3 border ${isDark ? 'bg-[#0D9488]/5 border-[#0D9488]/20' : 'bg-amber-50 border-amber-200'}`}>
                      {/* Client */}
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {linkedClient?.phone && (
                            <a href={`https://wa.me/55${linkedClient.phone.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" className="text-[9px] font-bold text-[#0D9488] hover:underline">
                              📞 {linkedClient.phone}
                            </a>
                          )}
                        </div>
                        ...
                        <div className="pt-2 border-t border-white/5 space-y-1">
                          ...
                          {linkedApp.additionals.map((ad: any) => (
                            <div key={ad.id} className="flex items-center justify-between">
                              ...
                              <span className="text-[10px] font-black text-[#0D9488]">R$ {(ad.price * ad.qty).toFixed(2)}</span>
                            </div>
                          ))}
                        </div>
                      )}
                      ...
                      {linkedApp.totalPrice && (
                        <div className={`flex items-center justify-between pt-2 border-t ${isDark ? 'border-[#0D9488]/20' : 'border-amber-200'}`}>
                          <span className={`text-[9px] font-black uppercase tracking-widest ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Total Cobrado</span>
                          <span className="text-sm font-black text-[#0D9488]">R$ {Number(linkedApp.totalPrice).toFixed(2)}</span>
                        </div>
                      )}
                      {/* Status */}
                      <div className="flex items-center justify-between">
