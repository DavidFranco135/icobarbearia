...
const REPLY_URL = 'https://us-central1-barbearia-do-ico.cloudfunctions.net/whatsappReply';
    ...
    <div className={`flex flex-col h-full w-full border-r ${divider}`}>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide" style={{WebkitOverflowScrolling:"touch", overflowY:"auto"}}>
        ...
        {filteredConvs.map(conv => (
          <div key={conv.id} className={`relative border-b group/conv ${divider} ${selectedConv?.id === conv.id ? (isDark ? 'bg-[#0D9488]/10 border-l-4 border-l-[#0D9488]' : 'bg-amber-50 border-l-4 border-l-[#0D9488]') : ''}`}>
            <button
              ...
              <div className={`w-11 h-11 rounded-full flex items-center justify-center text-base font-black shrink-0 relative ${isDark ? 'bg-[#0D9488]/20 text-[#0D9488]' : 'bg-amber-100 text-white'}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between gap-1 mt-0.5">
                  ...
                  {conv.unread && <span className="w-2.5 h-2.5 rounded-full bg-[#0D9488] shrink-0" />}
                </div>
              </div>
            </button>
            <button
        ...
        <>
          <div className={`px-4 py-3 flex items-center justify-between border-b shrink-0 ${divider}`}>
            <div className="flex items-center gap-2">
              ...
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-base shrink-0 ${isDark ? 'bg-[#0D9488]/20 text-[#0D9488]' : 'bg-amber-100 text-white'}`}>
                {resolveClientName(selectedConv)?.charAt(0)?.toUpperCase()}
              </div>
              <div>
                <p className={`font-black text-sm ${txt}`}>{resolveClientName(selectedConv)}</p>
                <a href={`https://wa.me/${selectedConv.clientPhone}`} target="_blank" rel="noreferrer" className={`text-[10px] flex items-center gap-1 hover:text-[#0D9488] transition-all ${sub}`}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-hide" style={{WebkitOverflowScrolling:"touch", overflowY:"auto"}}>
            ...
            {messages.map(msg => (
              <div key={msg.id} className={`flex ${msg.from === 'admin' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[82%] rounded-2xl overflow-hidden ${msg.from === 'admin' ? 'bg-[#0D9488] text-black rounded-br-sm' : isDark ? 'bg-white/10 text-white rounded-bl-sm' : 'bg-zinc-100 text-zinc-900 rounded-bl-sm'}`}>
                  {/* Imagem */}
          ...
          <div className={`p-3 border-t shrink-0 ${divider}`}>
            <div className="flex gap-2 items-end">
              <textarea
                ...
                className={`flex-1 resize-none rounded-2xl p-3 text-sm font-medium outline-none border transition-all ${isDark ? 'bg-white/5 border-white/10 text-white placeholder:text-zinc-600 focus:border-[#0D9488]/50' : 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-[#0D9488]'}`}
              />
