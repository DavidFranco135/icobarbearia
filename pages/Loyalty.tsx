  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 h-full overflow-auto scrollbar-hide">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Cartões Ativos', value: stats.totalCards, icon: CreditCard, color: '#0D9488' },
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`w-full border rounded-2xl py-4 pl-16 pr-8 text-sm focus:border-[#0D9488]/50 outline-none font-bold ${theme === 'light' ? 'bg-zinc-50 border-zinc-300 text-zinc-900' : 'bg-white/[0.03] border-white/10 text-white'}`}
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {clients
          ...
          .slice(0, 6)
          .map(client => (
            <div key={client.id} className={`rounded-[2rem] p-6 border border-dashed cursor-pointer hover:border-[#0D9488]/50 transition-all group ${theme === 'light' ? 'bg-zinc-50 border-zinc-300' : 'border-white/10 bg-white/[0.01]'}`}
              onClick={() => handleCreateCard(client.id)}>
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 rounded-2xl bg-zinc-900 border border-[#0D9488]/20 flex items-center justify-center font-black text-[#0D9488] text-lg">
              ...
              <p className="text-[10px] text-zinc-600 font-black uppercase tracking-widest text-center py-3 group-hover:text-[#0D9488] transition-colors">
        ...
        {enrichedCards.map(card => {
          ...
          return (
            <div
              ...
              className={`rounded-[2rem] p-6 border cursor-pointer hover:border-[#0D9488]/50 transition-all group ${themeCard}`}
            >
              {/* Header */}
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-[#0D9488]/10 flex items-center justify-center font-black text-[#0D9488] text-lg">
                ...
                {(card.freeCutsPending || 0) > 0 && (
                  <span className="bg-[#0D9488] text-black text-[9px] font-black px-3 py-1 rounded-full uppercase">
      ...
      {showManualModal && selectedCard && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/95 backdrop-blur-xl animate-in zoom-in-95">
          <div className={`w-full max-w-md rounded-[3rem] p-10 space-y-8 border shadow-2xl ${theme === 'light' ? 'bg-white border-zinc-200' : 'cartao-vidro border-[#0D9488]/20'}`}>
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-[#0D9488] text-black flex items-center justify-center text-2xl font-black">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className={`p-4 rounded-2xl text-center ${theme === 'light' ? 'bg-zinc-50 border border-zinc-200' : 'bg-white/5 border border-white/5'}`}>
                ...
                <p className="text-xl font-black text-[#0D9488]">{selectedCard.stamps || 0}<span className="text-xs">/{stampsForFreeCut}</span></p>
              </div>
              <div className={`p-4 rounded-2xl text-center ${theme === 'light' ? 'bg-zinc-50 border border-zinc-200' : 'bg-white/5 border border-white/5'}`}>
              ...
              <div className={`p-4 rounded-2xl text-center ${theme === 'light' ? 'bg-zinc-50 border border-zinc-200' : 'bg-white/5 border border-white/5'}`}>
                ...
                <p className="text-xl font-black text-teal-400">{selectedCard.freeCutsPending || 0}</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  { type: 'add_stamp' as const, label: '+ Selo', color: 'bg-[#0D9488] text-black' },
