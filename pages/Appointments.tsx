    ...
    <div className="h-full flex flex-col space-y-4 animate-in fade-in pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex gap-2 mt-2">
             <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg ${viewMode === 'grid' ? 'bg-[#0D9488] text-black' : 'bg-white/5 text-zinc-500'}`}><LayoutGrid size={16}/></button>
             <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg ${viewMode === 'list' ? 'bg-[#0D9488] text-black' : 'bg-white/5 text-zinc-500'}`}><List size={16}/></button>
             {viewMode === 'grid' && (
               <button 
                 onClick={() => {
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-1 flex-wrap">
            <button 
              ...
              className={`px-2.5 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${filterPeriod === 'day' ? 'bg-[#0D9488] text-black' : theme === 'light' ? 'bg-zinc-100 text-zinc-600' : 'bg-white/5 text-zinc-500'}`}
            >Dia</button>
            <button 
              onClick={() => setFilterPeriod('month')} 
              className={`px-2.5 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${filterPeriod === 'month' ? 'bg-[#0D9488] text-black' : theme === 'light' ? 'bg-zinc-100 text-zinc-600' : 'bg-white/5 text-zinc-500'}`}
            >Mês</button>
            <button 
              onClick={() => setFilterPeriod('all')} 
              className={`px-2.5 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${filterPeriod === 'all' ? 'bg-[#0D9488] text-black' : theme === 'light' ? 'bg-zinc-100 text-zinc-600' : 'bg-white/5 text-zinc-500'}`}
            >Todos</button>
            <button 
        ...
        {viewMode === 'grid' ? (
              ...
              <div className={`border-b sticky top-0 z-10 ${theme === 'light' ? 'border-zinc-200 bg-zinc-50' : 'border-white/5 bg-white/[0.02]'}`} style={{display:'grid', gridTemplateColumns: compactView ? `60px repeat(${professionals.length}, minmax(80px, 1fr))` : `80px repeat(${professionals.length}, minmax(120px, 1fr))`}}>
                ...
                {professionals.map(prof => (
                  <div key={prof.id} className={`flex items-center justify-center gap-3 border-r border-white/5 ${compactView ? 'p-2 flex-col' : 'p-3'}`}>
                    <img src={prof.avatar} className={`rounded-lg object-cover border border-[#0D9488] ${compactView ? 'w-6 h-6' : 'w-8 h-8'}`} alt="" />
                ...
                <div key={hour} className={`border-b ${theme === 'light' ? 'border-zinc-100' : 'border-white/[0.03]'} ${compactView ? 'min-h-[32px]' : 'min-h-[64px]'}`} style={{display:'grid', gridTemplateColumns: compactView ? `60px repeat(${professionals.length}, minmax(80px, 1fr))` : `80px repeat(${professionals.length}, minmax(120px, 1fr))`}}>
                  ...
                  {professionals.map(prof => {
                    ...
                    return (
                      ...
                      >
                        {app ? (
                          <div className={`h-full w-full rounded-2xl border flex flex-col justify-between transition-all group ${app.status === 'CONCLUIDO_PAGO' ? 'border-emerald-500/40 bg-emerald-500/10' : app.status === 'NAO_COMPARECEU' ? 'border-red-500/40 bg-red-500/10' : app.awaitingOnlinePayment ? 'border-blue-400/50 bg-blue-500/10' : 'border-[#0D9488]/30 bg-[#0D9488]/5'} ${compactView ? 'p-1.5 rounded-lg' : 'p-2'}`}>
                            <div className="truncate" onClick={(e) => { e.stopPropagation(); setShowDetailModal(app); }} style={{cursor:'pointer'}}>
                              <div className="flex items-center gap-1">
                                <h4 className={`font-black uppercase truncate hover:text-[#0D9488] transition-colors ${compactView ? 'text-[8px]' : 'text-[10px]'} ${app.status === 'NAO_COMPARECEU' ? 'text-red-400' : theme === 'light' ? 'text-zinc-900' : 'text-white'}`}
                                  title="Ver detalhes"
                                >{app.clientName}</h4>
                                ...
                                {app.awaitingOnlinePayment && app.status !== 'CONCLUIDO_PAGO' && (
                                  <span title="Aguardando pagamento online" className="animate-pulse flex-shrink-0">
                                    <CreditCard size={compactView ? 8 : 10} className="text-teal-400" />
                                  </span>
                                )}
                              ...
                              {!compactView && (
                                <>
                                  <p className="text-[8px] font-black text-[#0D9488] uppercase mt-0.5 truncate">{app.serviceName}</p>
                                  <p className="text-[7px] text-zinc-500 font-bold mt-0.5">{app.startTime}{app.endTime ? ` – ${app.endTime}` : ''}</p>
                                </>
                              )}
                              {compactView && <p className="text-[7px] text-[#0D9488]/70 truncate">{app.serviceName}</p>}
                            </div>
                          ...
                          <div className="h-full w-full flex items-center justify-center opacity-0 hover:opacity-40 transition-opacity">
                            <Plus size={compactView ? 12 : 16} className="text-[#0D9488]" />
                          </div>
                        )}
                      </div>
                    );
          ...
          <div className="p-6 space-y-3 overflow-y-auto h-full scrollbar-hide">
            ...
            {(filterPeriod as any) === 'espera' && (
              <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {(waitQueueStore || []).filter((w:any) => w.status === 'AGUARDANDO').map((w:any, idx:number) => (
                      <div key={w.id} className={`rounded-2xl border p-4 flex items-center gap-4 ${isDark ? 'bg-blue-500/5 border-blue-500/20' : 'bg-blue-50 border-blue-200'}`}>
                        <div className="w-9 h-9 rounded-xl bg-blue-500/20 flex items-center justify-center text-teal-400 font-black text-sm shrink-0">{idx + 1}</div>
                        ...
                        <div className="flex gap-2 shrink-0">
                          <button onClick={() => { setModoAvulso(true); setAvulsoNome(w.name); if(w.profId) setNewApp((prev:any)=>({...prev, professionalId:w.profId})); removeFromWaitQueue(w.id); setShowAddModal(true); }}
                            className="px-3 py-2 bg-[#0D9488] text-black rounded-xl font-black text-[9px] uppercase">✂️</button>
                ...
                <div className={`rounded-2xl p-4 border space-y-3 ${isDark ? 'bg-blue-500/5 border-blue-500/20' : 'bg-blue-50 border-blue-200'}`}>
                  <p className="text-[9px] font-black uppercase tracking-widest text-teal-400">⏳ Adicionar à fila de espera</p>
                  <input
                    type="text"
                    placeholder="Nome do cliente avulso *"
                    value={waitName}
                    onChange={e => setWaitName(e.target.value)}
                    className={`w-full border p-3 rounded-xl text-sm font-bold outline-none ${isDark ? 'bg-white/5 border-white/10 text-white placeholder:text-zinc-600 focus:border-[#0D9488]' : 'bg-white border-zinc-300 text-zinc-900 focus:border-[#0D9488]'}`}
                  />
                  <select
                    value={waitProfId}
                ...
                {waitList.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    ...
                    {waitList.map((w, idx) => {
                      ...
                      return (
                        <div key={w.id} className={`rounded-2xl border p-4 flex items-center gap-4 ${isDark ? 'bg-blue-500/5 border-blue-500/20' : 'bg-blue-50 border-blue-200'}`}>
                          <div className="w-9 h-9 rounded-xl bg-blue-500/20 flex items-center justify-center text-teal-400 font-black text-sm shrink-0">
                            {idx + 1}
                          </div>
                          <div className="flex-1 min-w-0">
                          ...
                          <div className="flex gap-2 shrink-0">
                            <button
                              ...
                              className="px-3 py-2 bg-[#0D9488] text-black rounded-xl font-black text-[9px] uppercase hover:bg-[#E5A86A] transition-all"
                            >
                              ✂️ Agendar
                            </button>
                            <button
            ...
            {filterPeriod !== 'fiados' && (
             ...
             {appointmentsFiltered.map(app => (
               <div key={app.id} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-[#0D9488]/30 transition-all">
                  <div className="flex items-center gap-4">
                     <div className={`w-10 h-10 rounded-xl border flex items-center justify-center ${app.status === 'CONCLUIDO_PAGO' ? 'border-emerald-500 text-emerald-500 bg-emerald-500/10' : app.awaitingOnlinePayment ? 'border-blue-400 text-teal-400 bg-blue-400/10' : 'border-amber-400 text-white bg-amber-400/10'}`}>
                        {app.status === 'CONCLUIDO_PAGO' ? <Check size={20}/> : app.awaitingOnlinePayment ? <CreditCard size={20} className="text-teal-400 animate-pulse"/> : <Banknote size={20} className="text-white animate-pulse"/>}
                     </div>
                     <div>
                        <p 
                          className="text-xs font-black cursor-pointer hover:text-[#0D9488] transition-colors"
                          onClick={() => setShowDetailModal(app)}
                          title="Ver detalhes do agendamento"
                        >{app.clientName} • <span className="text-[#0D9488]">{app.startTime}</span></p>
      ...
      {showRescheduleModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/95 backdrop-blur-xl animate-in zoom-in-95">
          <div className="cartao-vidro w-full max-w-sm rounded-[2.5rem] p-10 space-y-8 border-[#0D9488]/30 shadow-2xl">
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/95 backdrop-blur-xl animate-in zoom-in-95">
          <div className="cartao-vidro w-full max-w-lg rounded-[2.5rem] p-10 space-y-8 border-[#0D9488]/20 relative">
               ...
               <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button type="button"
                      ...
                      className={`py-3 rounded-xl font-black text-[10px] uppercase tracking-widest border-2 transition-all ${!modoAvulso ? 'border-[#0D9488] bg-[#0D9488]/10 text-[#0D9488]' : 'border-white/10 bg-white/5 text-zinc-500'}`}
                    >👤 Cadastrado</button>
                    <button type="button"
                  ...
                  <div className="relative">
                    <Search size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#0D9488] pointer-events-none"/>
                    <input
                      type="text"
                      placeholder="Buscar por nome, telefone ou e-mail..."
                      value={clientSearch}
                      onChange={e => { setClientSearch(e.target.value); setNewApp({...newApp, clientId: ''}); }}
                      className="w-full bg-white/5 border border-white/10 p-4 pl-10 rounded-xl outline-none text-xs font-black uppercase placeholder:normal-case placeholder:font-normal focus:border-[#0D9488] transition-all"
                    />
                  </div>
                  {clientSearch && (
                      ...
                      {clients
                        ...
                        .map((c: any) => (
                          <button key={c.id} type="button"
                            ...
                            className={`w-full text-left px-4 py-3 text-xs font-bold flex items-center justify-between transition-all border-b last:border-b-0 ${newApp.clientId === c.id ? 'bg-[#0D9488]/20 text-[#0D9488]' : isDark ? 'text-white hover:bg-white/5 border-white/5' : 'text-zinc-900 hover:bg-zinc-50 border-zinc-100'}`}
                          >
                            <span>{c.name}</span>
                  ...
                  <button type="button" onClick={() => setShowQuickClient(v => !v)}
                    className="w-full flex items-center justify-center gap-2 py-3 rounded-xl border border-[#0D9488]/40 text-[#0D9488] bg-[#0D9488]/5 hover:bg-[#0D9488]/15 transition-all font-black text-[10px] uppercase tracking-widest">
                    <UserPlus size={14}/> Cadastrar Novo Cliente
                  </button>
                  {showQuickClient && (
                    <div className="p-4 bg-white/5 rounded-xl border border-[#0D9488]/30 space-y-3 animate-in slide-in-from-top-2 max-h-80 overflow-y-auto">
                      <p className="text-[9px] font-black uppercase text-[#0D9488]">📋 Cadastro Rápido</p>
                      {/* Obrigatórios */}
                      ...
                      <div className="flex gap-2 pt-1">
                        ...
                        <button type="button" onClick={handleQuickClient} className="flex-1 bg-[#0D9488] text-black py-2 rounded-lg text-[9px] font-black uppercase">Salvar e Selecionar</button>
                      </div>
                    </div>
                  )}
                    </>
      ...
      {showDetailModal && (() => {
        ...
        return (
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-6 bg-black/95 backdrop-blur-xl animate-in zoom-in-95">
            <div className={`w-full max-w-md rounded-[2.5rem] shadow-2xl border flex flex-col max-h-[90vh] ${theme === 'light' ? 'bg-white border-zinc-200' : 'cartao-vidro border-[#0D9488]/20'}`}>
              {/* Header — fixo */}
              <div className="p-8 pb-0 flex items-start justify-between">
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-[#0D9488] mb-1">Detalhes do Agendamento</p>
              ...
              <div className="space-y-3">
                <div className={`p-4 rounded-2xl ${theme === 'light' ? 'bg-zinc-50' : 'bg-white/5'}`}>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <Scissors size={16} className="text-[#0D9488] shrink-0"/>
                    ...
                    {app.status !== 'CONCLUIDO_PAGO' && (
                      <button onClick={() => { setEditDetailService(v => !v); setEditDetailPrice(String(app.price || 0)); }}
                        className="text-[9px] font-black text-[#0D9488] hover:underline uppercase tracking-widest">
                  ...
                  {editDetailService ? (
                    <div className="space-y-3 animate-in slide-in-from-top-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        }} className="px-4 py-2 bg-[#0D9488] text-black rounded-xl font-black text-[10px] uppercase">Salvar</button>
                      </div>
                    </div>
                  ) : (
                    <>
                ...
                <div className={`flex items-center gap-4 p-4 rounded-2xl ${theme === 'light' ? 'bg-zinc-50' : 'bg-white/5'}`}>
                  <User size={16} className="text-[#0D9488] shrink-0"/>
                  <div>
                ...
                {client?.phone && (
                  <div className={`flex items-center gap-4 p-4 rounded-2xl ${theme === 'light' ? 'bg-zinc-50' : 'bg-white/5'}`}>
                    ...
                    <div>
                      ...
                      <a href={`https://wa.me/55${client.phone.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" className="text-sm font-black text-[#0D9488] hover:underline">{client.phone}</a>
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <p className="text-[9px] font-black uppercase tracking-widest text-zinc-500 flex items-center gap-2">
                        <Camera size={12} className="text-[#0D9488]"/> Fotos do Corte
                      </p>
                      <label className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl cursor-pointer text-[9px] font-black uppercase transition-all ${photoUploading ? 'opacity-50 pointer-events-none' : 'bg-[#0D9488]/10 text-[#0D9488] hover:bg-[#0D9488]/20'}`}>
                        <ImagePlus size={12}/>
                ...
                <>
                  ...
                  <div className={`flex items-center justify-between gap-3 p-4 rounded-2xl ${isDark ? 'bg-white/3 border border-white/5' : 'bg-zinc-50 border border-zinc-100'}`}>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <input
                        ...
                        className={`w-20 text-right font-black text-[#0D9488] text-sm border-b-2 outline-none bg-transparent transition-all ${isDark ? 'border-white/10 focus:border-[#0D9488]' : 'border-zinc-300 focus:border-[#0D9488]'}`}
                    ...
                    <div className="space-y-2">
                      ...
                      {finAdditionals.map(item => (
                        <div key={item.id} className={`flex items-center gap-3 p-3 rounded-xl ${isDark ? 'bg-white/3 border border-white/5' : 'bg-zinc-50 border border-zinc-100'}`}>
                          ...
                          <p className="text-sm font-black text-[#0D9488]">R$ {(item.price * item.qty).toFixed(2)}</p>
                  ...
                  <div className={`p-4 rounded-2xl space-y-3 ${isDark ? 'border border-white/5 bg-white/2' : 'border border-zinc-100 bg-zinc-50'}`}>
                    <div className="flex items-center justify-between">
                      ...
                      {clientVipDiscount > 0 && (
                        <span className="text-[9px] font-black px-2 py-1 rounded-full bg-[#0D9488]/20 text-[#0D9488] uppercase tracking-widest">
                  ...
                  {/* ── Total ── */}
                  <div className={`flex items-center justify-between p-4 rounded-2xl border ${isDark ? 'border-[#0D9488]/30 bg-[#0D9488]/5' : 'border-amber-200 bg-amber-50'}`}>
                    <p className={`font-black uppercase text-[10px] tracking-widest ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Total</p>
                    <p className="font-black text-xl text-[#0D9488]">R$ {finTotal.toFixed(2)}</p>
                  </div>
                  ...
                  {clientVipSub && (() => {
                    ...
                    return (
                      <div className={`p-4 rounded-2xl border-2 ${available ? 'border-[#0D9488]/50 bg-[#0D9488]/5' : 'border-red-500/30 bg-red-500/5'}`}>
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <Crown size={14} className="text-[#0D9488]"/>
                            <p className="text-[10px] font-black uppercase tracking-widest text-[#0D9488]">{plan.name}</p>
                          </div>
                          {available
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <button
                        ...
                        className={`flex flex-col items-center gap-1.5 p-4 rounded-2xl border-2 transition-all font-black text-[10px] uppercase tracking-widest ${finPayMethod === 'DEBITO' ? 'border-blue-500 bg-blue-500/10 text-teal-400' : isDark ? 'border-white/10 bg-white/5 text-zinc-500 hover:border-white/20' : 'border-zinc-200 bg-zinc-50 text-zinc-400 hover:border-zinc-300'}`}
                      >
                        <span className="text-2xl">💳</span>
                        Débito
                        {finPayMethod === 'DEBITO' && <span className="text-[8px] text-teal-400">Recebido na conta</span>}
                      </button>
                      {/* Crédito */}
                      <button
                        onClick={() => setFinPayMethod('CREDITO')}
                        className={`flex flex-col items-center gap-1.5 p-4 rounded-2xl border-2 transition-all font-black text-[10px] uppercase tracking-widest ${finPayMethod === 'CREDITO' ? 'border-purple-500 bg-purple-500/10 text-teal-400' : isDark ? 'border-white/10 bg-white/5 text-zinc-500 hover:border-white/20' : 'border-zinc-200 bg-zinc-50 text-zinc-400 hover:border-zinc-300'}`}
                      >
                        <span className="text-2xl">💳</span>
                        Crédito
                        {finPayMethod === 'CREDITO' && <span className="text-[8px] text-teal-400">Recebido na conta</span>}
                      </button>
                    </div>
