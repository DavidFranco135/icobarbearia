...
const GaleriaCortes: React.FC = () => {
  ...
  const inp  = `w-full border p-4 rounded-xl text-sm font-bold outline-none transition-all ${isDark ? 'bg-white/5 border-white/10 text-white placeholder:text-zinc-600 focus:border-[#0D9488]' : 'bg-zinc-50 border-zinc-300 text-zinc-900 placeholder:text-zinc-400 focus:border-[#0D9488]'}`;
  ...
  return (
    <div className="space-y-8 animate-in fade-in pb-20 h-full overflow-y-auto scrollbar-hide">
      ...
      {showNewAlbum && !activeAlbum && (
        <div className={`rounded-2xl p-6 border animate-in slide-in-from-top-2 space-y-4 ${isDark ? 'bg-[#0D9488]/5 border-[#0D9488]/20' : 'bg-amber-50 border-amber-200'}`}>
          <p className="text-[10px] font-black uppercase tracking-widest text-[#0D9488]">📁 Nome da Nova Pasta</p>
      ...
      {!activeAlbum && (
        albums.length === 0 ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
            {albums.map(album => (
              <div key={album.id} className={`rounded-[2rem] border overflow-hidden group cursor-pointer transition-all hover:scale-[1.02] hover:border-[#0D9488]/40 ${card}`} onClick={() => setActiveAlbumId(album.id)}>
                ...
                <div className="p-4">
                  {editingAlbumId === album.id ? (
                    <div className="flex gap-2" onClick={e => e.stopPropagation()}>
                      ...
                      <button onClick={() => handleRenameAlbum(album.id)} className="p-2 bg-[#0D9488] text-black rounded-lg"><Check size={12}/></button>
                    </div>
                  ) : (
                    <>
      ...
      {activeAlbum && (
        <>
          {uploading && (
            <div className={`rounded-2xl p-4 border ${isDark ? 'bg-[#0D9488]/5 border-[#0D9488]/20' : 'bg-amber-50 border-amber-200'}`}>
              <div className="flex items-center justify-between mb-2">
                <p className="text-[10px] font-black uppercase tracking-widest text-[#0D9488]">Enviando fotos ao ImgBB...</p>
                <p className="text-[10px] font-black text-[#0D9488]">{uploadProgress}%</p>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {activeAlbum.photos.map((photo, idx) => (
                <div key={idx} className="space-y-2">
                  ...
                  <input type="text" placeholder="Descrição (opcional)" value={photo.desc} onChange={e => handleUpdateDesc(idx, e.target.value)} className={`w-full border p-2 rounded-xl text-[10px] font-bold outline-none transition-all ${isDark ? 'bg-white/5 border-white/5 text-white placeholder:text-zinc-700 focus:border-[#0D9488]/50' : 'bg-zinc-50 border-zinc-200 text-zinc-900 placeholder:text-zinc-400 focus:border-[#0D9488]'}`}/>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="relative w-full max-w-lg px-4" onClick={e => e.stopPropagation()}>
            ...
            {photos.length > 1 && (
              <div className="flex justify-center gap-1.5 mt-4">
                {photos.map((_, i) => (
                  <button key={i} onClick={e => { e.stopPropagation(); setLightboxIdx(i); }} className={`rounded-full transition-all ${i === lightboxIdx ? 'w-4 h-2 bg-[#0D9488]' : 'w-2 h-2 bg-white/30'}`}/>
                ))}
              </div>
            )}
