...
const Schedule: React.FC = () => {
  ...
  const inp = `w-full border p-4 rounded-xl outline-none font-bold text-sm transition-all focus:border-[#0D9488] ${
    isDark
  ...
  const sel = `w-full border p-4 rounded-xl outline-none font-bold text-sm transition-all focus:border-[#0D9488] cursor-pointer ${
    isDark
  ...
  const mdl       = `w-full max-w-md rounded-[3rem] p-10 space-y-7 border shadow-2xl max-h-[90vh] overflow-y-auto scrollbar-hide ${isDark ? 'bg-[#181818] border-[#0D9488]/30' : 'bg-white border-zinc-200'}`;
      ...
      <div className="flex gap-2 flex-wrap">
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as any)}
            className={`flex items-center gap-2 px-5 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${activeTab === tab.id ? 'bg-[#0D9488] text-black' : isDark ? 'bg-white/5 text-zinc-500' : 'bg-zinc-100 text-zinc-600'}`}
          >
            ...
            return (
              <div key={prof.id} className={`rounded-[2rem] p-6 border ${bg}`}>
                <div className="flex items-center gap-4 mb-6">
                  <img src={prof.avatar} className="w-12 h-12 rounded-2xl object-cover border border-[#0D9488]/30" alt="" />
                  <div>
                  ...
                  <button onClick={() => { setBlockForm({ ...blockForm, professionalId: prof.id }); setShowBlockModal(true); }}
                    className="ml-auto p-2.5 bg-[#0D9488]/10 text-[#0D9488] hover:bg-[#0D9488] hover:text-black rounded-xl transition-all">
                    <Plus size={16} />
                  </button>
                </div>
                  ...
                  <div key={camp.id} className={`rounded-2xl p-5 border ${bg}`}>
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <MessageSquare size={20} className="text-[#0D9488] shrink-0" />
                        <div>
                  ...
                  {(inactivityCampaigns || [])
                    ...
                    .map((camp: InactivityCampaign) => (
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-2.5 rounded-xl font-black text-[9px] uppercase hover:scale-105 transition-all border ${isDark ? 'bg-white/5 border-white/10 text-zinc-300 hover:bg-[#0D9488]/20 hover:border-[#0D9488]/40' : 'bg-zinc-100 border-zinc-200 text-zinc-700 hover:bg-amber-50 hover:border-amber-300'}`}
                      >
          ...
          <div className={mdl}>
            ...
            <div className="space-y-5">
              ...
              <label className={`flex items-center gap-3 cursor-pointer ${txt}`}>
                <input type="checkbox" checked={blockForm.recurring} onChange={e => setBlockForm({ ...blockForm, recurring: e.target.checked })} className="w-5 h-5 rounded accent-[#0D9488]" />
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {DAYS.map((day, i) => (
                      <button key={i} type="button"
                        ...
                        className={`px-3 py-2 rounded-lg text-[9px] font-black uppercase ${blockForm.recurringDays.includes(i) ? 'bg-[#0D9488] text-black' : isDark ? 'bg-white/10 text-zinc-400' : 'bg-zinc-100 text-zinc-600'}`}
                      >{day}</button>
                    ))}
                  </div>
                </div>
