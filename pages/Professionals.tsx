      ...
      <button key={day} onClick={() => toggle(day)}
        className={`aspect-square rounded-xl flex items-center justify-center text-[11px] font-black transition-all hover:scale-110 active:scale-95 ${
          marked
            ...
            : today
              ? isDark ? 'bg-[#0D9488]/20 text-[#0D9488] border border-[#0D9488]/50' : 'bg-amber-100 text-white border border-amber-300'
  ...
  return (
    <div className="space-y-3">
      ...
      <div className={`flex items-center gap-4 flex-wrap text-[8px] font-black uppercase tracking-widest p-3 rounded-2xl ${isDark?'bg-white/[0.03]':'bg-zinc-50'}`}>
        ...
        <span className="flex items-center gap-1.5"><span className={`w-3 h-3 rounded inline-block border ${isDark?'bg-[#0D9488]/20 border-[#0D9488]/50':'bg-amber-100 border-amber-300'}`}/><span className={isDark?'text-zinc-400':'text-zinc-500'}>Hoje</span></span>
      </div>
...
const Professionals: React.FC = () => {
  ...
  const inputCls  = `w-full border p-5 rounded-2xl outline-none font-bold transition-all ${isDark?'bg-white/5 border-white/10 text-white focus:border-[#0D9488]':'bg-zinc-50 border-zinc-300 text-zinc-900 focus:border-[#0D9488]'}`;
    ...
    <div className="space-y-10 animate-in fade-in duration-500 h-full overflow-auto pb-24 sm:pb-10 scrollbar-hide">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {professionals.map(p => {
          ...
          return (
            <div key={p.id} className={`rounded-[2.5rem] p-10 group relative overflow-hidden border hover:border-[#0D9488]/40 transition-all duration-500 ${cardClass}`}>
              <div className="flex items-start justify-between">
                <div className="relative">
                  <img src={p.avatar} className="w-24 h-auto rounded-3xl object-contain border-2 border-white/10 group-hover:border-[#0D9488]/50 transition-all shadow-2xl block" alt={p.name}/>
                  <div className="absolute -bottom-2 -right-2 bg-[#0D9488] text-black p-2 rounded-xl shadow-xl"><Sparkles size={14}/></div>
                </div>
              ...
              <div className="mt-8">
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  ...
                  {(p as any).isMaster && (
                    <span className="inline-flex items-center gap-1 bg-[#0D9488]/20 text-[#0D9488] border border-[#0D9488]/40 text-[8px] font-black px-2 py-0.5 rounded-lg uppercase">★ Master{(p as any).masterSurcharge>0?` +R$${(p as any).masterSurcharge}`:''}</span>
                  )}
                  {(p as any).phone && (
              ...
              {ws && (
                <div className="mt-5 flex gap-1 flex-wrap">
                  {DAYS.map(d=>(
                    <span key={d.key} className={`text-[8px] font-black px-2 py-1 rounded-lg uppercase tracking-widest ${ws[d.key]?.active?'bg-[#0D9488]/20 text-[#0D9488] border border-[#0D9488]/30':isDark?'bg-white/5 text-zinc-600 border border-white/5':'bg-zinc-100 text-zinc-400 border border-zinc-200'}`}>
                      {d.label}
                    </span>
                  ))}
                </div>
              ...
              <div className="mt-5 grid grid-cols-3 gap-3">
                {[
                  {label:'Comissão', value:`${p.commission}%`,             color:'text-[#0D9488]'},
      ...
      {showModal && (
        <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-xl animate-in zoom-in-95 duration-300 ${isDark?'bg-black/95':'bg-black/70'}`}>
          <div className={`w-full max-w-2xl rounded-[3rem] p-8 md:p-12 space-y-8 relative max-h-[95vh] overflow-y-auto scrollbar-hide shadow-2xl ${isDark?'cartao-vidro border-[#0D9488]/10':'bg-white border border-zinc-200'}`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2 space-y-3">
                <button type="button" onClick={()=>setFormData(p=>({...p,isMaster:!(p as any).isMaster}))}
                  className={`w-full flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${(formData as any).isMaster?'border-[#0D9488] bg-[#0D9488]/10':isDark?'border-white/10 bg-white/5 hover:border-white/20':'border-zinc-200 bg-zinc-50 hover:border-zinc-300'}`}>
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg ${(formData as any).isMaster?'bg-[#0D9488] text-black':isDark?'bg-white/10 text-zinc-500':'bg-zinc-200 text-zinc-500'}`}>★</div>
                  ...
                  <div className={`w-12 h-6 rounded-full relative transition-colors ${(formData as any).isMaster?'bg-[#0D9488]':isDark?'bg-white/10':'bg-zinc-300'}`}>
                ...
                {([
                  ...
                  <button key={tab.id} onClick={()=>setScheduleTab(tab.id)}
                    className={`flex-1 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                      scheduleTab===tab.id
                        ? 'bg-[#0D9488] text-black shadow-md'
              ...
              {scheduleTab==='semana' && (
                <div className="space-y-3">
                  ...
                  {DAYS.map(d => {
                    ...
                    const day = ws[d.key];
                    return (
                      <div key={d.key} className={`flex items-center gap-2 p-2.5 rounded-2xl border transition-all ${day.active?(isDark?'bg-[#0D9488]/5 border-[#0D9488]/20':'bg-amber-50 border-amber-200'):(isDark?'bg-white/[0.02] border-white/5 opacity-50':'bg-zinc-50 border-zinc-200 opacity-50')}`}>
                        <button onClick={()=>toggleDay(d.key)} className={`w-11 shrink-0 text-[9px] font-black uppercase py-2 rounded-xl transition-all ${day.active?'bg-[#0D9488] text-black':isDark?'bg-white/10 text-zinc-500':'bg-zinc-200 text-zinc-500'}`}>
                          {d.label}
                        </button>
                        {day.active ? (
                          <div className="flex items-center gap-1.5 flex-1 min-w-0">
                            <Clock size={11} className="text-[#0D9488] shrink-0"/>
