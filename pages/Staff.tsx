...
const Staff: React.FC = () => {
  ...
  const roleLabel = (r: string) =>
    ...
    : r === 'RECEPCAO' ? { label: 'Recepção', color: 'text-teal-400 bg-blue-500/10 border-blue-500/20' }
    ...
    <div className="space-y-8 animate-in fade-in duration-500 h-full overflow-auto pb-20 scrollbar-hide">
            ...
            return (
              <div key={s.id} className={`${cardClass} rounded-[2.5rem] p-7 space-y-5 relative`}>
                ...
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#0D9488] to-[#8B5E2E] flex items-center justify-center text-black font-black text-2xl italic shrink-0">
                    {s.name.charAt(0)}
                  </div>
                  <div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl animate-in zoom-in-95">
          <div className={`w-full max-w-xl rounded-[3rem] shadow-2xl flex flex-col max-h-[92vh] ${isDark ? 'cartao-vidro border-[#0D9488]/10' : 'bg-white border border-zinc-200'}`}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <button
                      ...
                      className={`p-4 rounded-2xl border-2 text-left transition-all space-y-2 ${form.role === opt.key ? 'border-[#0D9488] bg-[#0D9488]/10' : isDark ? 'border-white/10 bg-white/5 hover:border-white/20' : 'border-zinc-200 bg-zinc-50 hover:border-zinc-300'}`}
                    >
                      <div className={`${form.role === opt.key ? 'text-[#0D9488]' : isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>{opt.icon}</div>
                  ...
                  {PAGES_ALL.map(pg => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-3 p-3 rounded-xl border transition-all text-left ${on ? (isDark ? 'border-[#0D9488]/40 bg-[#0D9488]/10' : 'border-amber-300 bg-amber-50') : (isDark ? 'border-white/5 bg-white/[0.02] hover:bg-white/5' : 'border-zinc-200 bg-zinc-50 hover:bg-zinc-100')}`}
                      >
                        <span className="text-base">{pg.icon}</span>
                        <span className={`text-[10px] font-black flex-1 ${on ? (isDark ? 'text-[#0D9488]' : 'text-white') : isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>{pg.label}</span>
                        {on && <Check size={12} className="text-[#0D9488] shrink-0"/>}
                      </button>
                    );
                  })}
                </div>
