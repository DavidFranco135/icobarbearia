...
const Subscriptions: React.FC = () => {
  ...
  const inp        = `w-full border p-4 rounded-xl outline-none font-bold text-sm transition-all
    ${isDark ? 'bg-zinc-900 border-white/10 text-white focus:border-[#0D9488]'
             : 'bg-zinc-50 border-zinc-300 text-zinc-900 focus:border-[#0D9488]'}`;
  ...
  const mdl        = `w-full max-w-md rounded-[3rem] p-10 space-y-7 border shadow-2xl
    ${isDark ? 'bg-[#111] border-[#0D9488]/30' : 'bg-white border-zinc-200'}`;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 h-full overflow-auto scrollbar-hide">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total',    value: stats.total,              icon: Users,       color: '#0D9488' },
            ...
            className={`relative flex items-center gap-1.5 px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all
              ${filterStatus === t.key
                ? t.key === 'A_VENCER'
                  ...
                  : 'bg-[#0D9488] text-black shadow-lg'
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div className="flex items-center gap-5 flex-1">
                    <div className="w-14 h-14 rounded-2xl bg-[#0D9488]/10 flex items-center justify-center text-2xl font-black text-[#0D9488] flex-shrink-0">
                  ...
                  <div className="text-center">
                    ...
                    {usagePct !== null && (
                      <div className={`mt-1 w-16 h-1.5 rounded-full ${isDark ? 'bg-white/10' : 'bg-zinc-200'}`}>
                        <div className="h-full bg-[#0D9488] rounded-full" style={{ width: `${usagePct}%` }} />
                      </div>
                    )}
                  </div>
                  <div className="text-center">
                    <p className="text-[8px] font-black uppercase text-zinc-500 mb-1">Valor</p>
                    <p className="font-black text-xl text-[#0D9488]">R$ {sub.price.toFixed(2)}</p>
                {(() => {
                          ...
                          return (
                            <div key={member.label}>
                              <div className="flex justify-between text-[9px] font-black uppercase mb-1">
                                ...
                                <span className={used >= max ? 'text-red-400' : 'text-[#0D9488]'}>{used}/{max}</span>
                              </div>
                              <div className={`w-full h-1.5 rounded-full ${isDark ? 'bg-white/10' : 'bg-zinc-200'}`}>
                                <div className={`h-full rounded-full transition-all ${used >= max ? 'bg-red-500' : 'bg-[#0D9488]'}`} style={{width: `${pct}%`}}/>
                              </div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    <button onClick={() => openEditSub(sub)}
                      ...
                      className={`p-3 rounded-xl border transition-all ${isDark ? 'bg-blue-500/10 border-blue-500/20 text-teal-400 hover:bg-blue-500/20' : 'bg-blue-50 border-blue-200 text-teal-400 hover:bg-blue-100'}`}>
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                {formData.planId && (() => {
                  ...
                  return (
                    <div className={`p-3 rounded-xl border mt-2 ${isDark ? 'border-[#0D9488]/20 bg-[#0D9488]/5' : 'border-amber-300 bg-amber-50'}`}>
                      <p className="text-[9px] font-black uppercase text-[#0D9488]">
