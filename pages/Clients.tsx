    ...
    <div className="space-y-8 animate-in fade-in duration-500 h-full overflow-auto pb-20 scrollbar-hide">
      ...
      <div className="relative">
        ...
        <input 
          ...
          className="w-full bg-white/[0.03] border border-white/10 rounded-2xl py-5 pl-16 pr-8 text-sm focus:border-[#0D9488]/50 outline-none transition-all placeholder:text-zinc-700 font-bold text-white"
        />
      </div>

      {/* ── Painel de Indicações Pendentes ── */}
      {(referrals || []).filter((r: any) => r.status === 'PENDENTE').length > 0 && (
        <div className={`rounded-2xl p-5 border ${theme === 'light' ? 'bg-amber-50 border-amber-200' : 'bg-[#0D9488]/10 border-[#0D9488]/25'}`}>
          ...
          <p className={`text-[10px] mb-3 ${theme === 'light' ? 'text-white' : 'text-[#0D9488]/80'}`}>
...
${r.referrerName} receberá R$ ${r.rewardAmount} na carteira.`)) validateReferral(r.id); }}
                ...
                <tr key={client.id} className="hover:bg-white/[0.02] transition-colors group cursor-pointer" onClick={() => setSelectedClient(client)}>
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-zinc-900 border border-[#0D9488]/20 flex items-center justify-center font-black text-[#0D9488] text-sm italic group-hover:bg-[#0D9488] group-hover:text-black transition-all">
                        {(client.name || "?").charAt(0)}
                      </div>
                      <div>
                        <p className="text-sm font-bold text-white group-hover:text-[#0D9488] transition-all">{client.name}</p>
      ...
      {selectedClient && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl animate-in zoom-in-95">
          <div className="cartao-vidro w-full max-w-2xl rounded-[3rem] p-8 md:p-12 space-y-8 border-[#0D9488]/10 relative shadow-2xl max-h-[85vh] overflow-hidden flex flex-col">
            <div className="flex justify-between items-start">
               <div className="flex items-center gap-5">
                  <div className="w-16 h-16 rounded-[1.5rem] bg-[#0D9488] text-black flex items-center justify-center text-3xl font-black italic">{(selectedClient.name || "?").charAt(0)}</div>
                  <div>
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hide space-y-8 pr-2">
               <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
                     ...
                     <p className="text-xl font-black text-[#0D9488] italic font-display">R$ {(selectedClient.totalSpent || 0).toFixed(2)}</p>
                  </div>
                  <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
               ...
               <div className="space-y-4">
                  <h3 className="text-[10px] font-black text-[#0D9488] uppercase tracking-[0.2em] mb-4 flex items-center gap-2"><CheckCircle2 size={14} /> Próximos Serviços</h3>
                  {clientAppointments.future.map(app => (
                    <div key={app.id} className="bg-white/5 border border-[#0D9488]/20 p-4 rounded-2xl flex items-center justify-between group hover:bg-[#0D9488]/5 transition-all">
                       <div className="flex items-center gap-4">
                          <div className="w-10 h-10 rounded-xl bg-zinc-900 border border-white/5 flex items-center justify-center text-[#0D9488]"><Calendar size={18}/></div>
                          <div>
                       ...
                       <div className="text-right">
                          ...
                          <p className="text-[8px] font-black uppercase tracking-widest mt-1 text-teal-400">AGENDADO</p>
                       </div>
                    </div>
                  ))}
               ...
               <div className="space-y-4 mt-2">
                  <div className="flex items-center justify-between">
                    <h3 className={`text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2 ${theme === 'light' ? 'text-zinc-700' : 'text-zinc-400'}`}>
                      <Camera size={14} className="text-[#0D9488]"/> Fotos dos Cortes
                    </h3>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl animate-in zoom-in-95 duration-300">
          <div className={`w-full max-w-lg rounded-[2.5rem] shadow-2xl flex flex-col max-h-[92vh] ${theme === 'light' ? 'bg-white border border-zinc-200' : 'cartao-vidro border-[#0D9488]/10'}`}>

            {/* Header */}
            <div className="px-8 pt-8 pb-4 flex items-center justify-between shrink-0">
              <div>
                <p className="text-[9px] font-black uppercase tracking-widest text-[#0D9488] mb-1">{editingId ? 'Editar Cadastro' : 'Novo Cliente'}</p>
            ...
            <div className="flex-1 overflow-y-auto px-8 pb-4 space-y-5 scrollbar-hide">
              ...
              <div className="space-y-2">
                ...
                <input type="text" placeholder="Nome completo *" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className={`w-full border p-4 rounded-2xl outline-none text-sm font-bold transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]/50'}`}/>
                <div className="grid grid-cols-2 gap-2">
                  <input type="tel" placeholder="WhatsApp *" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className={`w-full border p-4 rounded-2xl outline-none text-sm font-bold transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]/50'}`}/>
                  <input type="text" placeholder="CPF / CNPJ" value={formData.cpfCnpj} onChange={e => setFormData({...formData, cpfCnpj: e.target.value})} className={`w-full border p-4 rounded-2xl outline-none text-sm font-bold transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]/50'}`}/>
                </div>
                <input type="email" placeholder="E-mail" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className={`w-full border p-4 rounded-2xl outline-none text-sm font-bold transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]/50'}`}/>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input type="date" value={formData.birthdate} onChange={e => setFormData({...formData, birthdate: e.target.value})} className={`w-full border p-4 rounded-2xl outline-none text-sm font-bold transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]/50'}`}/>
                  </div>
                  <div>
                    <p className={`text-[8px] font-black uppercase mb-1 ml-1 ${theme === 'light' ? 'text-zinc-400' : 'text-zinc-600'}`}>Gênero</p>
                    <select value={formData.gender} onChange={e => setFormData({...formData, gender: e.target.value})} className={`w-full border p-4 rounded-2xl outline-none text-sm font-bold transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]/50'}`}>
                ...
                <input type="text" placeholder="Profissão" value={formData.profession} onChange={e => setFormData({...formData, profession: e.target.value})} className={`w-full border p-4 rounded-2xl outline-none text-sm font-bold transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]/50'}`}/>
              </div>

              {/* Endereço */}
              <div className="space-y-2">
                <p className={`text-[9px] font-black uppercase tracking-widest ${theme === 'light' ? 'text-zinc-400' : 'text-zinc-600'}`}>Endereço</p>
                <input type="text" placeholder="Logradouro e número" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} className={`w-full border p-4 rounded-2xl outline-none text-sm font-bold transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]/50'}`}/>
                <div className="grid grid-cols-2 gap-2">
                  <input type="text" placeholder="Bairro" value={formData.neighborhood} onChange={e => setFormData({...formData, neighborhood: e.target.value})} className={`w-full border p-4 rounded-2xl outline-none text-sm font-bold transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]/50'}`}/>
                  <input type="text" placeholder="Cidade" value={formData.city} onChange={e => setFormData({...formData, city: e.target.value})} className={`w-full border p-4 rounded-2xl outline-none text-sm font-bold transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]/50'}`}/>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <input type="text" placeholder="@instagram (sem @)" value={formData.instagram} onChange={e => setFormData({...formData, instagram: e.target.value})} className={`w-full border p-4 rounded-2xl outline-none text-sm font-bold transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]/50'}`}/>
                <select value={formData.howFound} onChange={e => setFormData({...formData, howFound: e.target.value})} className={`w-full border p-4 rounded-2xl outline-none text-sm font-bold transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]/50'}`}>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full border p-4 pr-12 rounded-2xl outline-none text-sm font-bold transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-200 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]/50'}`}
                  />
                  <button
                    type="button"
