  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-700 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className={`text-3xl md:text-4xl font-black font-display tracking-tight flex items-center gap-3 ${isDark ? 'text-white' : 'text-zinc-900'}`}>
            Gestão <span className="text-[#0D9488] italic">Ico</span>
          </h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Hoje', value: revenue.daily, color: '#0D9488', sub: 'Faturamento diário' },
          ...
          <div
            ...
            className={`${cardClass} p-6 rounded-[2rem] cursor-pointer group hover:border-[#0D9488]/40 transition-all duration-300 relative overflow-hidden`}
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        {[
          ...
          { label: 'Novos Clientes/Mês', value: newClientsMonth, icon: Users, color: '#0D9488' },
        ].map((item) => (
          <div
            key={item.label}
            onClick={() => onNavigate('appointments')}
            className={`${cardClass} p-5 rounded-2xl cursor-pointer hover:border-[#0D9488]/30 transition-all group`}
          >
          ...
          <div className="space-y-3 max-h-[260px] overflow-y-auto scrollbar-hide">
            {todayApps.map(app => (
              <div
                key={app.id}
                onClick={() => onNavigate('appointments')}
                className={`flex items-center gap-3 p-3 rounded-2xl cursor-pointer transition-all group border border-transparent hover:border-[#0D9488]/20 ${isDark ? 'hover:bg-white/5' : 'hover:bg-zinc-50'}`}
              >
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-black text-sm transition-all group-hover:bg-[#0D9488] group-hover:text-black ${isDark ? 'bg-zinc-900 text-[#0D9488]' : 'bg-zinc-100 text-[#0D9488]'}`}>
            ...
            <div className="space-y-3">
              {serviceRanking.map((svc, i) => (
                <div key={svc.name} className="flex items-center gap-4">
                  <span className={`text-[10px] font-black w-5 ${i === 0 ? 'text-[#0D9488]' : subTextClass}`}>#{i + 1}</span>
