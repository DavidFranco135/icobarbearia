...
const FilaEspera: React.FC = () => {
  ...
  const inp = `w-full border p-4 rounded-2xl text-sm font-bold outline-none transition-all ${isDark ? 'bg-white/5 border-white/10 text-white placeholder:text-zinc-600 focus:border-[#0D9488]' : 'bg-zinc-50 border-zinc-300 text-zinc-900 placeholder:text-zinc-400 focus:border-[#0D9488]'}`;
    ...
    <div className={`min-h-screen flex flex-col items-center justify-center p-6 ${bg}`}>
      ...
      {step === 'waiting' && (
        <div className={`w-full max-w-sm rounded-[2.5rem] p-8 space-y-6 text-center ${card}`}>
          {/* Número na fila */}
          <div>
            <div className="w-24 h-24 rounded-full gradiente-ouro flex items-center justify-center mx-auto mb-4 shadow-2xl shadow-[#0D9488]/30">
