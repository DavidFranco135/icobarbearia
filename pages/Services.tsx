    ...
    <div className="space-y-6 animate-in fade-in pb-10 h-full overflow-y-auto scrollbar-hide">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="flex-1 relative">
          ...
          <input type="text" placeholder="Pesquisar Serviços..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className={`w-full border py-3 pl-12 pr-4 rounded-xl text-xs font-black outline-none transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-300 text-zinc-900 placeholder:text-zinc-400 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]'}`} />
        </div>
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          {categories.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest whitespace-nowrap border transition-all ${activeCategory === cat ? 'bg-[#0D9488] text-black border-transparent' : theme === 'light' ? 'bg-white border-zinc-200 text-zinc-600' : 'bg-white/5 text-zinc-500 border-white/5'}`}>{cat}</button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredServices.map(s => (
          <div key={s.id} className={`p-4 rounded-[2rem] border flex gap-4 transition-all group ${theme === 'light' ? 'bg-white border-zinc-200 shadow-sm hover:border-[#0D9488]' : 'cartao-vidro border-white/5 hover:border-[#0D9488]/40'}`}>
              ...
              <div>
                ...
                {s.description && s.description.length > 50 && (
                  <button 
                    ...
                    className="text-[9px] font-black text-[#0D9488] hover:text-[#0D9488]/80 mt-1 transition-all"
                  >
                    {expandedDescriptions[s.id] ? 'Ver menos' : 'Ver mais'}
                  </button>
                )}
                <div className="flex gap-3 mt-2">
                   <span className="text-[9px] font-black text-[#0D9488]">R$ {s.price}</span>
              ...
              <div className="space-y-6 pb-4">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2 space-y-1">
                    ...
                    <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className={`w-full border p-4 rounded-xl text-xs font-bold outline-none transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-300 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]'}`} />
                  </div>
                  <div className="space-y-1">
                    ...
                    <input required type="number" step="0.01" value={formData.price} onChange={e => setFormData({...formData, price: parseFloat(e.target.value)})} className={`w-full border p-4 rounded-xl text-xs font-bold outline-none transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-300 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]'}`} />
                  </div>
                  <div className="space-y-1">
                    ...
                    <input required type="number" value={formData.durationMinutes} onChange={e => setFormData({...formData, durationMinutes: parseInt(e.target.value)})} className={`w-full border p-4 rounded-xl text-xs font-bold outline-none transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-300 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]'}`} />
                  </div>
                  <div className="md:col-span-2 space-y-1">
                    ...
                    <input required type="text" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} className={`w-full border p-4 rounded-xl text-xs font-bold outline-none transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-300 text-zinc-900 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]'}`} />
                  </div>
                  <div className="md:col-span-2 space-y-1">
                    ...
                    <textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className={`w-full border p-4 rounded-xl text-xs font-bold resize-none h-20 outline-none transition-all ${theme === 'light' ? 'bg-zinc-50 border-zinc-300 text-zinc-900 placeholder:text-zinc-400 focus:border-[#0D9488]' : 'bg-white/5 border-white/10 text-white focus:border-[#0D9488]'}`} placeholder="Descreva o serviço..."></textarea>
                  </div>
               </div>
              </div>
            </div>
