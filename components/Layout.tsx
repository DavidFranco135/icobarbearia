              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-4 px-4 py-4 rounded-2xl transition-all duration-300 group ${
                    isActive 
                      ? 'gradiente-ouro text-white shadow-lg shadow-[#0D9488]/20' 
                      : theme === 'light' 
                        ? 'text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900' 
                        : 'text-zinc-500 hover:bg-white/5 hover:text-white'
                  }`}
                >
                  <Icon size={22} className={`flex-shrink-0 ${isActive ? 'text-black' : theme === 'light' ? 'text-zinc-700 group-hover:text-zinc-900' : 'text-zinc-500 group-hover:text-[#0D9488]'} transition-colors`} />
        ...
        <header className={`h-20 flex items-center justify-between px-6 md:px-10 border-b ${theme === 'light' ? 'bg-white border-zinc-200 shadow-sm' : 'bg-[#0A0A0A] border-white/5'}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <button onClick={() => setShowNotifs(!showNotifs)} className={`p-3 rounded-2xl border transition-all relative ${theme === 'light' ? 'bg-zinc-100 border-zinc-200 text-zinc-700 hover:bg-zinc-200 hover:border-zinc-300' : 'bg-white/5 border-white/10 text-zinc-400 hover:text-[#0D9488]'}`}>
                <Bell size={18} />
              ...
              {showNotifs && (
                <>
                  ...
                  <div className={`absolute right-0 mt-4 w-80 rounded-[2rem] shadow-2xl z-50 border overflow-hidden animate-in slide-in-from-top-2 ${theme === 'light' ? 'bg-white border-zinc-200' : 'bg-[#111111] border-white/10'}`}>
                    <div className={`p-6 border-b flex justify-between items-center bg-[#0D9488]`}>
                    ...
                    <div className="max-h-96 overflow-y-auto scrollbar-hide">
                       ...
                       {notifications.map(n => (
                         <div key={n.id} onClick={() => { markNotificationAsRead(n.id); setShowNotifs(false); setActiveTab('appointments'); }} className={`p-6 border-b cursor-pointer transition-all ${theme === 'light' ? 'border-zinc-200 hover:bg-zinc-50' : 'border-white/5 hover:bg-white/5'} ${!n.read ? 'bg-[#0D9488]/5 border-l-4 border-l-[#0D9488]' : ''}`}>
                            <p className="text-xs font-black text-[#0D9488]">{n.title}</p>
