      ...
      <div className={`w-full max-w-2xl rounded-t-[3rem] sm:rounded-[3rem] border max-h-[92vh] flex flex-col ${isDark ? 'bg-[#0A0A0A] border-white/10' : 'bg-[#F8F9FA] border-zinc-200'}`}>
        ...
        <div className={`flex items-center justify-between p-6 border-b ${isDark ? 'border-white/10' : 'border-zinc-200'}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest text-[#0D9488]">Clube de Benefícios 🎁</p>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide p-6 space-y-6">
                      ...
                      <div
                        ...
                        className={`rounded-2xl p-5 border transition-all ${
                          isAvailable || isQrGerado
                            ? isDark
                              ? 'border-[#0D9488]/40 bg-[#0D9488]/5 cursor-pointer hover:bg-[#0D9488]/10'
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className={`font-black text-sm ${isAvailable || isQrGerado ? 'text-[#0D9488]' : isDark ? 'text-zinc-400' : 'text-zinc-600'}`}>
                    ...
                    {activePartners.map(p => (
                      ...
                      >
                        {p.logo ? (
                          ...
                          <div className="w-10 h-10 rounded-xl bg-[#0D9488]/20 flex items-center justify-center mx-auto mb-2 text-lg">
                            {CATEGORY_ICONS[p.category || 'Outro'] || '🏪'}
                          </div>
                        )}
                        <p className={`text-[10px] font-black truncate ${txt}`}>{p.businessName || p.name}</p>
                        <p className="text-[9px] text-[#0D9488] font-black mt-0.5">
                          {clientVipDiscount > 0
          ...
          {step === 'partners' && (
            <>
              {selectedBenefit && (
                <div className={`p-4 rounded-2xl border border-[#0D9488]/30 bg-[#0D9488]/5 flex items-center gap-3`}>
                  <Gift size={20} className="text-[#0D9488] flex-shrink-0" />
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-widest text-[#0D9488]">Benefício Selecionado</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {activePartners.map(p => (
                    <button
                      ...
                      disabled={loading}
                      className={`rounded-2xl border p-5 text-left transition-all hover:border-[#0D9488]/50 group ${isDark ? 'bg-white/5 border-white/10 hover:bg-white/10' : 'bg-white border-zinc-200 hover:bg-amber-50'}`}
                    >
                      {/* Imagem do parceiro */}
                      {p.image && (
                      ...
                      <div className="flex items-start gap-3">
                        {p.logo ? (
                          ...
                          <div className="w-10 h-10 rounded-xl bg-[#0D9488]/20 flex items-center justify-center flex-shrink-0 text-xl">
                            {CATEGORY_ICONS[p.category || 'Outro'] || '🏪'}
                          </div>
                        )}
          ...
          {step === 'qr' && selectedPartner && (
            <div className="flex flex-col items-center gap-6 text-center">
              {qrError ? (
                ...
                <>
                  {/* Info do parceiro */}
                  <div className={`w-full p-5 rounded-2xl border flex items-center gap-4 ${isDark ? 'border-[#0D9488]/30 bg-[#0D9488]/5' : 'border-amber-300 bg-amber-50'}`}>
                    <div className="text-2xl">{CATEGORY_ICONS[selectedPartner.category || 'Outro'] || '🏪'}</div>
                    <div className="text-left">
                      <p className="font-black text-[#0D9488] text-sm">{selectedPartner.businessName || selectedPartner.name}</p>
                  ...
                  {/* QR Code */}
                  <div className="bg-white p-6 rounded-[2rem] shadow-2xl shadow-[#0D9488]/20">
                    <img
