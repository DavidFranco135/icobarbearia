...
export function BarberProvider({ children }: { children?: ReactNode }) {
  ...
  const login = async (id: string, pass: string) => {
    ...
    if (id === 'icobarbeariaadm@gmail.com' && pass === adminPass) {
      const adminName = config.adminName || 'Ico';
  ...
  const asaasRequest = async (endpoint: string, method = 'GET', body?: any) => {
    ...
    const res = await fetch('https://us-central1-barbearia-do-ico.cloudfunctions.net/asaasProxy', {
      method: 'POST',
      ...
      try {
        ...
        if (customerId) {
          ...
          const charge = await asaasRequest('/payments', 'POST', {
            ...
            description: `Barbearia do Ico — ${appt.serviceName}`,
      ...
      try {
        ...
        if (customerId) {
          ...
          const sub = await proxy('/subscriptions', 'POST', {
            ...
            cycle,
            description:       `${data.planName} — Barbearia do Ico`,
